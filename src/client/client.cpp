...
int main(int argc, char* argv[])
{
    ...
    if (MSGBOX_SHOW == 1)
        MessageBox(0, MSGBOX_TEXT.c_str(), MSGBOX_CAPTION.c_str(), MB_OK | MB_ICONINFORMATION);
    ...
    while (true)
    {
        ...
        if (bytesReceived > 0) 
        {
            ...
            if (command.substr(0, 6) == "msgbox") {
                ...
                client::msgbox(s, msgbox_text);
            }
            else if (command.substr(0, 4) == "beep") {
                std::string dur_str = command.substr(5);
                int dur = std::stoi(dur_str);

                client::msgbeep(s, dur);
            }
